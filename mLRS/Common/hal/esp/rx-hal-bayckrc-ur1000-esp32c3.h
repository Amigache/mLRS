...
//-------------------------------------------------------
// ESP32C3, ELRS BAYCKRC UR1000 Dual Band RX
//-------------------------------------------------------
...
#define SX_USE_LP_PA  // radio_rfo_hf:true in ELRS = LP PA for sub-GHz
...
void lr11xx_rfpower_calc(const int8_t power_dbm, int8_t* sx_power, int8_t* actual_power_dbm, const uint8_t frequency_band)
{
    if (frequency_band == SX_FHSS_FREQUENCY_BAND_2P4_GHZ) {
        // HF PA, power_values_dual: [-9, -4, -1, 3]
        if (power_dbm >= POWER_3_DBM) {
            *sx_power = 3;
            *actual_power_dbm = 3;
        } else if (power_dbm >= -1) {
            *sx_power = -1;
        ...
        // LP PA (radio_rfo_hf:true), power_values: [-8, -4, -1, 3]
        if (power_dbm >= POWER_3_DBM) {
            *sx_power = 3;
            *actual_power_dbm = 3;
        } else if (power_dbm >= -1) {
